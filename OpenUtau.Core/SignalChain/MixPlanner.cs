...
namespace OpenUtau.Core {
    ...
    public sealed class MixPlanner {
        ...
        /// <summary>
        /// Whether the part can serve a full, correct extraction (DAW gate) or an
        /// authoritative piano-roll mix. When <paramref name="currentHashes"/> is
        /// supplied, completeness is content-keyed: every current hash must be in
        /// <see cref="completeParts"/> and the content cache. The playback session
        /// must not override that — after an edit the session can still hold Ready
        /// slots for stale hashes until the next <see cref="BeginSession"/>.
        /// Without hashes, falls back to session slot readiness for transport hold.
        /// </summary>
            ...
            lock (lockObj) {
                if (currentHashes != null) {
                    ...
                    foreach (var h in hashes) {
                        if (!done.Contains(h) || !TryGetCache(part, h, out _)) {
                            return false;
                        }
                    }
                    return true;
        ...
        /// <summary>
        /// Drops the completeness mark for a part so the next document-keyed
        /// <see cref="IsPartReady"/> is false until a render pass calls
        /// <see cref="MarkPartComplete"/>. Does not clear content-cache PCM.
        /// </summary>
        public void InvalidatePartCompleteness(UPart part) {
            lock (lockObj) {
                completeParts.Remove(part);
            }
        }
