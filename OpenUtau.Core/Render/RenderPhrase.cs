...
namespace OpenUtau.Core.Render {
    ...
    public class RenderPhone {
        ...
        internal RenderPhone(Pipeline.PhraseSource source, Pipeline.PhonemeSource phoneme, int phrasePosition) {
            ...
            blendPhoneme = phoneme.BlendPhoneme;
            blendWeight = phoneme.BlendWeight;
            tone = phoneme.Tone;
    ...
    public class RenderPhrase {
        ...
        /// <summary>
        /// Precomputed phrase audio layout in absolute project milliseconds,
        /// including the leading pre-utter and the release tail, matching the
        /// slot layout used by the mix.
        /// </summary>
        ...
        internal RenderPhrase(Pipeline.PhraseSource source, int phraseStart, int phraseEnd) {
            ...
            for (int d = 0; d < source.CurveDescriptors.Length; ++d) {
                var descriptor = source.CurveDescriptors[d];
                var curve = source.Curves.FirstOrDefault(c => c.Abbr == descriptor.abbr)
                    ?? Pipeline.CurveSource.Empty(
                        descriptor.abbr, source.CurveDescriptorDefaults[d], (float)descriptor.min);
