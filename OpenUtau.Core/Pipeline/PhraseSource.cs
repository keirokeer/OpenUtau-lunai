...
namespace OpenUtau.Core.Pipeline {
    ...
    public sealed class CurveSource {
        ...
        /// <summary>Erase breaks; sampling across one yields <see cref="DefaultY"/>.</summary>
        public readonly int[] Breaks;
        /// <summary>
        /// The value an unedited position samples as: the track's effective
        /// expression default, resolved at snapshot time.
        /// </summary>
        ...
        internal CurveSource(UCurve curve, int defaultY) {
            ...
            Xs = curve.xs.ToArray();
            Ys = curve.ys.ToArray();
            Breaks = curve.breaks?.ToArray() ?? Array.Empty<int>();
            DefaultY = defaultY;
        ...
        internal CurveSource(string abbr, int[] xs, int[] ys, int[] breaks, int defaultY, float min, bool isEmpty) {
            Abbr = abbr;
            Xs = xs;
            Ys = ys;
            Breaks = breaks;
        ...
        internal static CurveSource Empty(string abbr, int defaultY, float min) =>
            new CurveSource(abbr, Array.Empty<int>(), Array.Empty<int>(), Array.Empty<int>(),
                defaultY, min, true);
        ...
        public bool HasBreakBetween(int x0, int x1) {
            if (Breaks.Length == 0 || x0 >= x1) {
                return false;
            }
            int lo = Array.BinarySearch(Breaks, x0 + 1);
            if (lo < 0) {
                lo = ~lo;
            }
            return lo < Breaks.Length && Breaks[lo] < x1;
        }
    }
    ...
    public sealed class PhonemeSource {
        ...
        /// <summary>Optional DiffSinger blend target phoneme; null = off.</summary>
    ...
    public sealed class PhraseSource {
        ...
        /// <summary>
        /// The effective (track-override aware) empty value of each entry of
        /// <see cref="CurveDescriptors"/>, resolved on the UI thread.
        /// </summary>
        ...
        internal PhraseSource(
            ...
            Curves = part.curves
                .Select(c => new CurveSource(
                    c, Util.ExpressionDefaultResolver.GetEffectiveDefaultInt(project, track, c.abbr)))
                .ToArray();
